 ...
 */
function getFileEntries(opt: { contentDir: string; assetDir: string; assetAlias: string }) {
  ...
  for (const entry of assetsEntries) {
    ...
    if (IMAGE_EXTS.has(ext)) {
      const alias = opt.assetAlias.endsWith("/") ? opt.assetAlias : opt.assetAlias + "/"
 ...
 */
function handleEmbed(target: string, resolvedPath: string | undefined, alias: string | undefined) {
