...
const remarkRelativePath: Plugin<[], Root> = () => {
  return (tree: Root, file: any) => {
    ...
    if (!isCurrentFileIndex) {
      const currentFileNameWithoutExt = path.posix.basename(currentFile).replace(RE_MD_EXT, "")
    ...
    visit(tree, "link", (node: Link, index, parent: any) => {
      ...
      let resolvedRelativePath = path.posix.relative(currentDirInBuild, targetPathInBuild)
