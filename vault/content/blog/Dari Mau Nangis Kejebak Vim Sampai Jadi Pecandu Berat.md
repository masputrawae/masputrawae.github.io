...
Cerita bermula di awal tahun 2022 (_seingat ku_ 😅). Waktu itu aku terpaksa mencoba Linux Debian, dan justru menginstal versi minimal—tanpa DE (desktop environment). Biasanya aku selalu mengandalkan `nano` sebagai teks editor CLI yang lebih friendly. Tapi entah kenapa, bukannya menginstal `nano`, aku malah iseng mengetik `vi .bashrc` (karena katanya bawaan Linux pasti ada `vi` / `vim` jadul) hanya untuk setting konfigurasi dikit.
